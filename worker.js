...
<head>
  ...
  <body>
    ...
    <table style="margin-top:12px; width:100%; border-collapse:collapse;">
      <thead>
        <tr>
          <th style="text-align:left; border-bottom:1px solid #eaecef; padding:6px 4px;">序号</th>
    ...
    <script>
      ...
      function parseUserState(bodyText) {
        const raw = String(bodyText || '').trim();
        if (!raw) return { banned: '未知', punishTitle: null, secUidVal: '' };
        const normalized = raw.replace(/^\uFEFF/, '').replace(/^for\s*\(\s*;;\s*\);\s*/, '');
        ...
        const suMatch = normalized.match(/"sec_uid"\s*:\s*"([^"]*)"/);
        const punishTitle = ptMatch ? ptMatch[1] : null;
        const secUidVal = suMatch ? suMatch[1] : '';
        if (punishTitle) return { banned: punishTitle === '账号已被封禁' ? '是' : '否', punishTitle, secUidVal };
        if (/账号已被封禁/.test(normalized)) return { banned: '是', punishTitle: '账号已被封禁', secUidVal };
        if (/"status_code"\s*:\s*0/.test(normalized) || /"user_info"\s*:/.test(normalized)) return { banned: '否', punishTitle: null, secUidVal };
        return { banned: '未知', punishTitle: null, secUidVal };
      }
      ...
      function updateConcurrency() {
        ...
        const validCount = lines.filter(line => isDigits(line)).length;
        if (validCount === 0) return;
        const suggestion = Math.min(Math.max(Math.ceil(validCount / 10), 1), 50);
        batchConcurrencyEl.value = suggestion;
      }
      batchInput.addEventListener('input', updateConcurrency);
      ...
      function ensureRow(index, id) {
        ...
        tr.innerHTML =
          '<td style="padding:6px 4px; border-bottom:1px solid #eaecef;">' + seq + '</td>' +
      ...
      async function queryOne(task, opts) {
        ...
        async function attempt() {
          ...
          let secUidVal = '';
          if (httpStatus === 200) {
            const parsed = parseUserState(bodyText);
            punishTitle = parsed.punishTitle;
            banned = parsed.banned;
            secUidVal = parsed.secUidVal;
          }
